	...
	const runRefactorCommand = vscode.commands.registerCommand(
      ...
      `;
			...
			try {
					...
					if (message.command === 'accept') {
						try {
							...
							vscode.window.showErrorMessage(
								`Failed to refactor ${message.filename}: ${err instanceof Error ? err.message : String(err)
								}`
							);
						}
				...
				panel.webview.html = `<p style="color:red;padding:1rem;">❌ Refactor failed: ${error instanceof Error ? error.message : String(error)
					}</p>`;
			}
		}
	);
